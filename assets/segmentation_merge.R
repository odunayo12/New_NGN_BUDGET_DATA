budget_18_19_20 <- rbind(data_pbi_2018, data_pbi_2019_start, data_pbi_2020_start)


projects_Unique <- as_tibble(budget_18_19_20) %>% 
  filter(!is.na(projectCode)) %>% 
  select(table_identifier, table_identifier_MDA, costCenter_Code, Year, projectCode,  Data.Column2, Data.Column3, Data.Column4) %>% 
  rename(project_description = Data.Column2, Status = Data.Column3, Amount = Data.Column4) %>% 
  filter( Year != 2019) %>% 
